...
const useController = ({ data, width, height }) => {
  ...
  const xMax = useMemo(
    () => d3.min(data, ({ items }) => d3.max(items, ({ date }) => date)),
    [data]
  );
  const slide = 120
  const xMinSliding = xMax < slide ? 0 : xMax-slide
  const xScale = useMemo(
    // () => d3.scaleTime().domain([xMin, xMax]).range([0, width]),
    // [xMin, xMax, width]
    () => d3.scaleLinear().domain([xMinSliding, xMax]).range([0, width]),
    [xMinSliding, xMax, width]
  );
  const yMin = useMemo(
